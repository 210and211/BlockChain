...
public class FileFragmentation_delete {//在开始删除操作时得进行block_Fragmentationdelete_us的标识为1 结束时赋0
...
//	public 
	FileFragmentation_delete(int blockchain_high,int port_Fragmentation_socket,ArrayList<String> IP){
		try {
	...
	void send(){
		while(block_Fragmentationdelete_using==1){
			...
			for(int i=0;i<peer_number;i++){
				Thread socketThread=new Thread(new QuestThread(ip_list.get(i),port_Fragmentation_socket,blockchain_high,i));
				socketThread.start();
			}
		}
		for(int i=0;i<peer_number;i++){
			Thread socketThread=new Thread(new ClientThread(ip_list.get(i),port_Fragmentation_socket,blockchain_high,i));
			socketThread.start();
		}
	}
	int get_exit_number(){
		return peer_exist_number;
	}
	class ClientThread extends Thread{
		...
		int count;
		ClientThread(String ip,int port,int blockchain_high,int count){
			this.ip=ip;
			this.port=port;
	...
	class QuestThread extends Thread{
		...
		String blockchain_high;//区块高度
		int count;
		QuestThread(String ip,int port,int blockchain_high,int count){
			this.ip=ip;
			this.port=port;
			this.blockchain_high=String.valueOf(blockchain_high);
			this.count=count;
	  ...
	  public void FileFragmentation(int high,int port,ArrayList<String> IP_list){//输入参数为区块高度
	    	//根据时间和节点个数进行删除
	    	//需要知道该文件的创造时间和该文件在p2p网络中所拥有的节点个数
	    	int multiple=0;//计算概率时的时间倍数
	    	int number=IP_list.size()-1;
	    	FileFragmentation_delete checking=new FileFragmentation_delete(high, port, IP_list);//广播全网得到存在节点的个数
	    	checking.send();
	    	int exit_number=checking.get_exit_number();
